...
# Program was intended to assist in answering some CSS questions inwhich
# contained binary-to-decimal conversions.
...
def init_num_list(interval: int) -> list:
    """
    Creates "num_list". The numbers inside the list will be used
    to align with each binary digit.
    """
    num_list = []
    ...
    # Subtracts old interval by 1 then appends the original number
    # to the first index for a correct "num_list".
    new_interval = interval-1
    i = 1
    while i <= new_interval:
        # Summation occurs until "i" exceeds "new_interval"
        # Appends resulting summation values to the "num_list".
        i += 1
        num += num
        num_list.append(num)
    return num_list
...
def align_to_decimal(binary: str, num_list: list) -> list:
    """Matches the binary digits with their correspoding values in num_list."""
    alignment = []

    for value, digit in zip(num_list, reversed(binary)):
        tmp = [value, digit]
        alignment.append(tmp)
    print(f"Before removing zeroes: {alignment}")
    return alignment


def filter_digit_alignment(alignment):
    """
    Removes the unnecessary alignments then creates the final version of
    "alignment" list only containing decimals.
    """
    ...
    # Finally, extracts the remaining values from "alignment"
    # and inserts to "final_alignment" to add all numbers.
    final_alignment = []
    for row in alignment:
        for items in row:
            final_alignment.append(items)
...
def combine_values(final_alignment):
    """
    The answer is finally obtained after the program sums the
    contents of the entire list.
    """
...
if __name__ == "__main__":
    # Before immediately proceeding into converting the binary,
    # a verification process takes place.
    #
    # The verification checks for non-binary digits within  the given binary.
    while True:
